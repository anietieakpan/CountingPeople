...
M6_DATA = Path(os.getenv('DATA_DIR', '../../data'), 'category4')

M6_DATA = Path(os.getenv('DATA_DIR', '.'), 'category4')
M6_IMAGES = Path(M6_DATA, 'images')
M6_MODELS = Path(M6_DATA, 'models')

# ### 1) Selecting an appropriate my_model
#
# GluonCV has a wide range pre-trained models so selecting the correct my_model for the task at hand can be tricky. As you can see below there are close to 200 different pre-trained my_model variants. You first task will be to select a good my_model given a few constraints.


for model in gcv.model_zoo.get_model_list():
    print(model)

# Our objective is to count people so certain types of my_model are better suited for this task than others. We also want a resource efficient my_model so let's say that the selected my_model must consume <1GB of memory. And lastly our my_model must be fast and be able to process >100 images per second on the [GluonCV website](https://gluon-cv.mxnet.io/model_zoo) benchmarks. Given these criteria, you first task is to select the most appropriate my_model for the task from the following list.
...
assert selected_model in set(model_options), 'You must choose from my_model_options'
assert hashlib.sha1(selected_model.encode('utf-8')).hexdigest()[:8] in ['9c7846f3',
                                                                        '1e6e3a3d'], 'You should select an object detection my_model for this task.'
assert hashlib.sha1(selected_model.encode('utf-8')).hexdigest()[
       :8] == '1e6e3a3d', 'You should choose a faster object detection my_model.'
...
# Your first task is to implement a function that loads an image from disk given a f_dir.
#
...
def load_image(filepath):
    ...
    :param filepath: relative or absolute f_dir to RGB image file in JPG format.
    :type filepath: str
...
# assert unnorm_image.shape == (1000, 1000, 1000 )
...
#
# You'll notice that the count_object function
# has
# a
# threshold
# parameter
...
# this
# amount
# for a detected person, you should add one to the total person count.
#
# **Hint**: it's possible to use MXNet NDArray functions to do this calculations without loops.
#
# # **Hint**: All 3 network outputs are avaliable in the function (i.e.
# class_ids
# ,
# scores
# ,
...
# ### 6) Count People
#
# In this section, you should stack all of the previously defined functions together and complete the implementation of the
# count
# method
# to
# return the
...
# assert counter.count(Path(MY_IMAGES, 'fun_company.jpg')) == 4
...
# assert counter.count(Path(MY_IMAGES, 'fun_company.jpg'), visualize=True) == 6
...
counter.count(Path(M6_IMAGES, 'fun_company.jpg'), visualize=True)
...
#
# In this final assignment of the course, you used a pre-trained object detection my_model to count people in a dataset of images.
#
# Although we are not eliminating false positives, the my_model was able to identify over 70 people in the dataset.
#
# You've now completed Compu